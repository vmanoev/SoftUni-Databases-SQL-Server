...
--View syntax
CREATE VIEW v_EmployeesSalaryInfo
AS
...
SELECT NEXT VALUE
FOR seq_Customers_CustomerID MINVALUE 10 MAXVALUE 10, 50 CYCLE

-- UPDATE Addresses
SET TownID IS NULL
WHERE TownID = 1 DDL, DML

ALTER,

UPDATE CTRL + K + C TO comment CTRL + R TO
...
-- Problem 6.
SELECT FirstName + '' + '.' + LastName + '@softuni.bg' AS full_email_address
FROM Employees;

SELECT CONCAT (FirstName, '.', LastName, '@softuni.bg') AS full_email_address1
FROM Employees;
...
AS
SELECT CONCAT_WS(' ', FirstName, ISNULL(MiddleName, ''), LastName) AS [FullName], JobTitle
FROM Employees
